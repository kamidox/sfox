    ...
    if( ssl->resumed == 0 )
    {
        ...
        if( ssl->minor_ver == SSLV3_MINOR_VERSION )
        {
            for( i = 0; i < 3; i++ )
            {
                ...
                md5_finish_x( &md5, ssl->master + i * 16 );
            }
        }
        else
...
int ssl_read_record( ssl_context *ssl, int do_crypt )
{
    ...
    ret = net_recv( ssl->read_fd, ssl->in_hdr + ssl->in_left, &len );
    ssl->in_left += len;
...
int ssl_read( ssl_context *ssl, unsigned char *buf, int *len )
{
    ...
    while( ssl->in_left < ssl->in_msglen )
    {
        if( ( ret = ssl_read_record( ssl, 1 ) ) != 0 )
            return( ret );
