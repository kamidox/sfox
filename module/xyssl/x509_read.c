...
*/
static int asn1_get_time( unsigned char **p, unsigned char *end, x509_time *t )
{
	int ret, len, utc;
	char date[5], *pd;
	...
	{
		utc = 0;
		if( ( ret = asn1_get_tag( p, end, &len, ASN1_GEN_TIME ) ) != 0 )
			return( ERR_X509_CERT_INVALID_DATE | ret );
	}
	memset( date,  0, sizeof( date ) );
	pd = *p;
	/* year */
	memcpy( date, pd, utc ? 2 : 4 );
	pd += (utc ? 2 : 4);
	t->year = atoi( date );
	/* month */
	memcpy( date, pd, 2 );
                           ...
                           x509_time *to )
{
    ...
    if( ( ret = asn1_get_time( p, end, from ) ) != 0 )
        return( ERR_X509_CERT_INVALID_DATE | ret );

    if( ( ret = asn1_get_time( p, end, to ) ) != 0 )
        return( ERR_X509_CERT_INVALID_DATE | ret );

    if( *p != end )
        return( ERR_X509_CERT_INVALID_DATE | ERR_ASN1_LENGTH_MISMATCH );
...
int x509_is_cert_expired( x509_cert *crt )
{
    ...
    if( lt.year  > crt->valid_to.year )
        return( BADCERT_HAS_EXPIRED );

    if( lt.year == crt->valid_to.year &&
        lt.month > crt->valid_to.mon - 1 )
        return( BADCERT_HAS_EXPIRED );

    if( lt.year == crt->valid_to.year &&
        lt.month == crt->valid_to.mon - 1 &&
        lt.day > crt->valid_to.day )
        return( BADCERT_HAS_EXPIRED );
